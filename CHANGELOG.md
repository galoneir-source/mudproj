...
### Cambiado
- `buscar_destino()` (`systems/fast_travel/fast_travel.py`) alineada con el patrón exacta-comprueba-ambigüedad ya usado por el resto de la familia `buscar_*` (`buscar_mazmorra`, v0.71.50; `buscar_receta`, v0.71.48). Antes, la rama de coincidencia exacta por nombre de sala devolvía el primer destino cuyo `nombre_sala` coincidiera con el texto (insensible a mayúsculas) sin comprobar si había más de un candidato -a diferencia de la rama de coincidencia parcial, justo debajo, que sí lo hace. No era explotable hoy: las 20 salas del catálogo de cartografía (`ZONAS_INFO`) tienen todas nombres de sala distintos, así que ningún jugador puede provocar la ambigüedad con el catálogo actual. Pero era la misma trampa de mantenimiento latente ya cerrada en mazmorras: en cuanto dos zonas compartan nombre de sala (p. ej. una "Cripta Helada" en dos áreas distintas), `viajar` a ese nombre exacto habría resuelto en silencio al primer destino en orden de lista en vez de avisar de la ambigüedad. Encontrado auditando la familia `buscar_*` del proyecto durante una revisión completa. Fix: la coincidencia exacta ahora también exige que haya exactamente un candidato, igual que la parcial. 1 test de regresión nuevo en `tests/test_fast_travel_system.py` (dos destinos inyectados con el mismo nombre de sala).
- `buscar_quest()` (`systems/quests/quests.py`) tenía el mismo hueco que `buscar_destino` (justo arriba): la rama de coincidencia exacta por título (`nombre_lower == q["titulo"].lower()`) devolvía la primera quest que coincidiera, sin comprobar ambigüedad, mientras que las dos ramas de coincidencia parcial que siguen sí lo hacen. No era explotable hoy (los 21 títulos de `QUESTS` son todos distintos). Encontrado en la misma auditoría de la familia `buscar_*`. Fix: la rama de título exacto ahora también exige exactamente un candidato antes de devolverlo, y devuelve `(None, None)` si hay dos o más. 1 test de regresión nuevo en `tests/test_quests_system.py` (dos quests inyectadas con el mismo título vía `unittest.mock.patch.dict`).
- `buscar_habilidad()` (`systems/skills/engine.py`) tenía el mismo hueco: la rama de coincidencia exacta por nombre de display devolvía la primera habilidad que coincidiera, sin comprobar ambigüedad, mientras que la rama de coincidencia parcial que sigue sí lo hace. No era explotable hoy (los 24 nombres de `HABILIDADES` son todos distintos). Encontrado en la misma auditoría. Fix: la rama de nombre exacto ahora también exige exactamente un candidato, y devuelve `(None, None)` si hay dos o más. 1 test de regresión nuevo en `tests/test_skills_system.py` (dos habilidades inyectadas con el mismo nombre vía `unittest.mock.patch.dict`).
- `buscar_runa()` (`systems/runes/runes.py`) tenía el mismo hueco en su propia rama de coincidencia exacta (por ID o por nombre) -no cubierta por el fix de v0.71.57, que solo cerró la rama de coincidencia parcial de esta misma función-: devolvía la primera runa cuyo ID o nombre coincidiera exactamente, sin comprobar si había más de una candidata. No era explotable hoy (los 8 IDs de `RUNAS` son únicos por ser claves de dict, y los 8 nombres de display tampoco se repiten). Encontrado revisando la misma función tras cerrar su hueco de coincidencia parcial, dentro de la auditoría de la familia `buscar_*`. Fix: la rama exacta ahora también exige exactamente un candidato, con el mismo criterio que la parcial. 1 test de regresión nuevo en `tests/test_runas_system.py` (dos runas inyectadas con el mismo nombre vía `monkeypatch.setitem`).

### Corregido
- `_buscar_npc_en_sala()` (`features/quests/commands.py`), usada por `aceptar` y `entregar` para exigir que el NPC dador/receptor esté presente, aceptaba cualquier objeto de la sala -no solo NPCs- y en ambas direcciones de subcadena (`buscado in key or key in buscado`). La dirección inversa hacía que cualquier objeto o jugador cuyo nombre fuera subcadena del NPC contara como ese NPC: un jugador llamado "Mira", "Gareth", "Torben", "Aldric" o "Guardia" actuaba como dador y receptor portátil de todas las misiones de "Mira la mercader", "Gareth el mesonero", etc., en cualquier sala donde estuviera -incluidas las de entrega de objetos-, y el diálogo mostraba al jugador hablando como el NPC. Encontrado auditando `features/quests/`, uno de los sistemas que nunca había aparecido en este CHANGELOG, durante una revisión completa. Fix: solo cuentan objetos de typeclass `NPC` sin cuenta, y solo si su key contiene el nombre buscado (los 5 dadores/receptores de `QUESTS` coinciden exactamente con la key de su prototipo). 2 tests de regresión nuevos en `tests/test_quests.py` (fallan sin el fix).
- `ExpedicionScript.abandonar()` (`features/expeditions/expedition_script.py`), invocado por `expedicion abandonar`, tenía exactamente el mismo hueco que acaba de corregirse en mazmorras: tampoco comprobaba si el jugador que abandona seguía en el `CombatHandler` activo de la sala de expedición antes de teletransportarlo de vuelta con `_teleportar_a_origen()`. El caso del último miembro ya lo cubre `_limpiar()` (que sí termina el combate antes de borrar la sala, igual que ante el timeout de 30 minutos), pero abandonar con compañeros de grupo aún dentro dejaba el `CombatHandler` huérfano -atascado esperando el turno de alguien que ya no está- y a ese jugador con `en_combate=True` para siempre; la sala de expedición tampoco tiene `Exit`s reales, así que `huir` nunca es alternativa ahí dentro y `abandonar` era la única vía de escape real a mitad de combate. Encontrado en la misma auditoría de movimientos forzados que el fix de mazmorras. Fix: mismo patrón, nuevo helper `ExpedicionScript._salir_de_combate_si_procede()` (limpia el estado de combate del jugador vía `_limpiar_estado_combate()` + `eliminar_participante()`, igual que hace `huir` dentro de un combate) llamado antes de `_teleportar_a_origen()`. 1 test de regresión nuevo en `tests/test_expeditions.py` (falla sin el fix: el jugador queda con `en_combate=True` tras abandonar).
- `MazmorraScript.salir()` (`features/dungeons/dungeon_script.py`), invocado por el subcomando `mazmorra salir`, movía al jugador a la sala de vestíbulo con un `move_to()` directo sin comprobar antes si seguía siendo participante del `CombatHandler` activo de su sala actual. `mazmorra entrar` ya comprueba `en_combate` en todo el grupo antes de teletransportar (v0.71.50) precisamente porque mover a alguien fuera de una sala mientras un `CombatHandler` lo sigue teniendo como participante deja ese combate huérfano -atascado esperando el turno de alguien que ya no está- y a ese jugador con `en_combate=True` para siempre; pero nadie había auditado el camino de salida simétrico. El caso en que abandona el último miembro de la instancia ya quedaba cubierto porque entonces se dispara `_limpiar()`, que sí termina correctamente cualquier combate activo en las salas antes de borrarlas (mismo mecanismo que el timeout de 3600s, v0.71.53); el hueco real aparecía cuando quedaba algún compañero de grupo todavía dentro de la mazmorra, ya que entonces `_limpiar()` nunca se disparaba. Las salas temporales de mazmorra no tienen `Exit`s reales, así que la acción `huir` de dentro de un combate (que busca una salida con `.destination` en el contenido de la sala) nunca funciona en una instancia ("no hay salida por donde escapar"), lo que convertía a `mazmorra salir` en la única vía de escape real a mitad de un combate -y era precisamente esa vía la que dejaba el rastro roto. Encontrado auditando el patrón de comprobación de `en_combate` en movimientos forzados durante una revisión completa del proyecto. Fix: nuevo helper `MazmorraScript._salir_de_combate_si_procede()` que, si el jugador sigue en el `CombatHandler` activo de su sala, lo saca de forma limpia (`_limpiar_estado_combate()` + `eliminar_participante()`, el mismo mecanismo que ya usa `CombatHandler._intentar_huida()` para la acción `huir`) antes de moverlo; se descartó bloquear `mazmorra salir` mientras `en_combate=True` -como sí hacen las comprobaciones de entrada- porque al no haber `Exit`s en las salas de mazmorra eso habría dejado al jugador completamente atrapado sin ninguna forma de abandonar una pelea que no puede ganar. 1 test de regresión nuevo en `tests/test_mazmorras.py` (falla sin el fix: el jugador queda con `en_combate=True` tras salir).
- `AuctionScript`, `CombatHandler`, `PatrolScript`, `ContractScript`, `DesafiosDiariosScript`, `EventoMundialScript`, `GuildWarScript`, `RecordsScript`, `RespawnScript` y `WorldBossScript` fijaban `self.interval` sin `self.start_delay = True` -mismo patrón de scripts persistentes ya corregido antes en mazmorras, torneos, intercambio, ticks de estado de combate, reloj mundial y clima (v0.71.3). Sin `start_delay`, Evennia arranca el `LoopingCall` con `now=True` y dispara el primer `at_repeat()` de forma síncrona en el instante de crear/añadir el script -antes incluso de que retorne la llamada que lo creó- en vez de esperar el primer intervalo completo. El efecto más visible estaba en `CombatHandler` (`features/combat/handler.py`): cada combate nuevo adelantaba `turno_tiempo` de 0 a 1 en el instante mismo de creación, restando 1s de los `TURNO_TIMEOUT` (15s) prometidos para el primer turno. En `PatrolScript` (`features/combat/patrol.py`) el efecto era peor y recurrente: un NPC de patrulla recién spawneado o respawneado (`programar_respawn()`, en cada muerte, no solo una vez) se teletransportaba de inmediato a la siguiente sala de patrulla en el mismo instante de añadir el script, en vez de esperar los 30s del primer tick. En los siete scripts globales de un solo tick por evento (subastas, contratos, desafíos diarios, eventos mundiales, guerras de gremio, marcadores y jefes de mundo) el efecto quedaba oculto por ser solo observable en la primerísima creación del script (arranque original del mundo o cualquier entorno de test/desarrollo nuevo), igual que ya se documentó para reloj mundial y clima. Encontrado ampliando el barrido sistemático de este patrón a los scripts restantes del proyecto que fijan `interval` sin haber sido auditados todavía; `expedicion_script.py` (el único script periódico restante) no lo necesita porque su `at_repeat()` ya está protegido por el flag `db.activo`, que `iniciar()` no activa hasta después de crear el script. Fix: `self.start_delay = True` en los 10 scripts. 12 tests de regresión nuevos en `tests/test_auctions.py`, `tests/test_contratos.py`, `tests/test_daily.py`, `tests/test_eventos.py`, `tests/test_guild_wars.py`, `tests/test_handler.py`, `tests/test_jefes_mundo.py`, `tests/test_records.py` y `tests/test_spawn.py` (respawn y patrulla).
...
- `systems/daily/daily.py` era el único módulo de `systems/` que usaba sintaxis de unión PEP 604 (`str | None`) en anotaciones de firma de función sin `from __future__ import annotations` en la cabecera. Esas anotaciones se evalúan al definir la función, así que bajo Python 3.9 —el intérprete con el que se resuelve `pytest` en el entorno de desarrollo local— la importación del módulo abortaba con `TypeError: unsupported operand type(s) for |: 'type' and 'NoneType'`, rompiendo la recolección de `tests/test_daily_system.py` (78 tests) y con ella cualquier `pytest tests/ -k system` completo. En producción (Python 3.11, requisito de Evennia 6.x) la sintaxis es válida y el sistema de Desafíos Diarios nunca se vio afectado. Los ~30 módulos restantes de `systems/` que usan esta sintaxis ya importaban `annotations` del futuro; `daily.py` era el único olvido. Encontrado ejecutando la suite pura durante una revisión del proyecto. Fix: se añade `from __future__ import annotations` al principio de `systems/daily/daily.py`. Sin cambios de comportamiento en producción; verificado que `pytest tests/ -k system` pasa entero (2349 tests) y que `generar_desafios_del_dia`, `formatear_desafios`, `racha_si_completa_hoy` y `formatear_racha` siguen devolviendo los mismos valores bajo 3.11 vía `evennia shell` en producción tras el despliegue.
...
### Cambiado
- `buscar_mazmorra()` (`systems/dungeons/dungeons.py`) alineada con el patrón exacta→parcial-con-ambigüedad de `buscar_receta()` (`systems/alchemy/alchemy.py`, v0.71.48) y `CmdComprar` (`features/shop/commands.py`). Antes: tras fallar la coincidencia por ID exacto, recorría los nombres y devolvía la **primera** mazmorra cuyo nombre contuviera el texto —sin comprobar coincidencia exacta de nombre ni ambigüedad—, y solo el fallback final por `startswith()` de ID chequeaba "un único candidato". No era explotable hoy (solo 3 mazmorras, "Cripta de Ceniza" / "Forja Maldita" / "Abismo Sin Fondo", sin nombres que sean prefijo o subcadena unos de otros), pero era la misma trampa de mantenimiento latente que ya se cerró en alquimia y tienda: en cuanto se añadiera una mazmorra cuyo nombre fuese prefijo de otra (p. ej. "Cripta Helada" y "Cripta Helada Profunda"), escribir el nombre corto y exacto se habría tratado como ambiguo, y un texto que coincidiera parcialmente con dos habría resuelto en silencio a la primera en orden de dict. Encontrado auditando el patrón de búsqueda parcial durante una revisión del proyecto. Fix: `buscar_mazmorra()` comprueba ahora (1) ID exacto, (2) nombre exacto insensible a mayúsculas —resuelve solo si hay exactamente una—, (3) subcadena de nombre o de ID —resuelve solo si hay exactamente un candidato, y devuelve `(None, None)` si hay dos o más. Sin cambio de comportamiento observable con el catálogo actual (`pytest tests/test_mazmorras_system.py` y `evennia test tests.test_mazmorras` siguen en verde). 3 tests de regresión nuevos en `tests/test_mazmorras_system.py` que inyectan mazmorras con nombres solapados vía `monkeypatch` para fijar la prioridad de la coincidencia exacta y el `(None, None)` ante ambigüedad; el antiguo `test_ambiguedad_startswith_devuelve_none` (cuyo propio comentario admitía que no probaba lo que decía) se sustituye por `test_prefijo_id_no_coincidente_devuelve_none`.

## [0.71.53] — 2026-09-03

### Corregido
- **El suite de integración completo (`evennia test --settings settings.py .`) no arrancaba.** `tests/test_bounty.py` (desde el 2026-08-29, `342c3c9`) y `tests/test_jefes_mundo.py` (desde el 2026-07-12, `a33b52e`) declaraban ambos, a nivel de módulo, `class JugadorDePrueba(Character)`. Los typeclasses de Evennia registran un modelo Django al crear la clase, y dos con el mismo nombre en la app `objects` chocan (`RuntimeError: Conflicting 'jugadordeprueba' models in application 'objects'`), abortando la colección de tests antes de ejecutar ninguno. Pasó desapercibido ~5 días porque el flujo por-fichero (`evennia test ... tests.test_bounty`) —el que documenta `CLAUDE.md`— nunca carga los dos módulos en el mismo proceso, y ninguna pasada completa se ejecuta de forma regular. Todos los demás módulos de test ya evitan la colisión con nombres únicos (`JugadorDePruebaArena`, `JugadorDePruebaCartografia`, `JugadorFalso`, `JugadorFalsoTurno`, `PersonajeConectado`). Encontrado al intentar correr la suite completa para revisar el proyecto. Fix: `tests/test_bounty.py::JugadorDePrueba` → `JugadorDePruebaBounty`. Sin cambios en código de producción; verificado que `tests.test_bounty` + `tests.test_jefes_mundo` ahora cargan y pasan juntos.
- `NPC._reaccionar_a_presencia()` (`typeclasses/npc.py`) ignoraba las penalizaciones de percepción por hora y clima. La v0.71.49 añadió aquí la comprobación `PerceptionManager.puede_detectar()` para que el sigilo protegiera contra el primer golpe de un NPC, pero la llamaba sin `hora=` ni `clima=` — era el único de los tres consumidores de `puede_detectar()` que lo hacía. Tanto `percibir` (`commands/general_commands.py`) como el listado de la sala (`Room.return_appearance`) calculan la hora y el clima de juego (bloque guardado por `room.db.exterior`) y los pasan, aplicando así las penalizaciones de percepción nocturna (`-3`) y climática (`-4` con niebla, `-2` tormenta, apilables). Efecto: en una sala exterior de noche o con niebla, un jugador con sigilo cuyo `nivel_sigilo` quedaba justo por encima de la percepción diurna del NPC seguía siendo detectado y agredido al entrar — la noche y la niebla, que solo deberían ayudar a quien se oculta, no ofrecían ninguna protección contra la agresión de un NPC, el mismo escenario que la v0.71.49 pretendía cerrar. Encontrado revisitando percepción/sigilo. Fix: `_reaccionar_a_presencia()` calcula `hora_juego`/`clima_juego` igual que `percibir` (guardado por `sala.db.exterior`) y los pasa a `puede_detectar()`. 4 tests de regresión nuevos en `tests/test_perception.py` (2 fallan sin el fix: noche y niebla).
