...
class PerceptionManager:
    ...
    def nivel_percepcion(self, observer: Any) -> int:
        """
        Devuelve el nivel de percepción del observador.
        Fórmula: inteligencia + nivel // 2
        ...
        return int(inteligencia + nivel // 2)

    # ------------------------------------------------------------------ #
    #  Detalles ocultos de sala
    # ------------------------------------------------------------------ #

    def revelar_detalles(self, room: Any, observer: Any) -> list[str]:
        """
        ...
        nivel = self.nivel_percepcion(observer)
        return [
            d["texto"]
            for d in detalles
    ...
    def puede_detectar(self, observer: Any, obj: Any) -> bool:
        """
        ...
        return self.nivel_percepcion(observer) >= nivel_sigilo

    def filtrar_visibles(self, observer: Any, objetos: list) -> list:
        """
        Filtra una lista de objetos devolviendo solo los que el observador
        puede detectar.
        """
        return [o for o in objetos if self.puede_detectar(observer, o)]
